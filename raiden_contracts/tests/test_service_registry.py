...
def test_deposit(
    ...
    custom_token.functions.approve(
        service_registry.address, 5000 * (10 ** 18)
    ).call_and_transact({"from": A})

    # happy path
    old_balance = custom_token.functions.balanceOf(A).call()
    current_price = service_registry.functions.current_price().call()
    service_registry.functions.deposit(5000 * (10 ** 18)).call_and_transact({"from": A})
    assert old_balance > custom_token.functions.balanceOf(A).call() > old_balance - current_price
...
def test_setURL(custom_token: Contract, service_registry: Contract, get_accounts: Callable) -> None:
    (A,) = get_accounts(1)
