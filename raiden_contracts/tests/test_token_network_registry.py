...
def test_constructor_call(
    ...
    with pytest.raises(TransactionFailed, match="invalid SR address"):
        get_token_network_registry(
    ...
    with pytest.raises(TransactionFailed, match="invalid SR"):
        get_token_network_registry(
    ...
    with pytest.raises(TransactionFailed, match="invalid settle timeout min"):
        get_token_network_registry(
            _secret_registry_address=secret_registry_contract.address,
            _chain_id=chain_id,
            _settlement_timeout_min=0,
    ...
    with pytest.raises(TransactionFailed, match="invalid settle timeout max"):
        get_token_network_registry(
            _secret_registry_address=secret_registry_contract.address,
            _chain_id=chain_id,
            _settlement_timeout_min=settle_min,
    ...
    with pytest.raises(TransactionFailed, match="invalid settle timeouts"):
        get_token_network_registry(
            _secret_registry_address=secret_registry_contract.address,
            _chain_id=chain_id,
            _settlement_timeout_min=settle_max,
    ...
    with pytest.raises(TransactionFailed, match="invalid TN limit"):
        get_token_network_registry(
            _secret_registry_address=secret_registry_contract.address,
            _chain_id=chain_id,
            _settlement_timeout_min=settle_min,
...
def test_create_erc20_token_network_call(
    ...
    with pytest.raises(TransactionFailed, match="invalid token address"):
        token_network_registry_contract.functions.createERC20TokenNetwork(
            EMPTY_ADDRESS,
            channel_participant_deposit_limit,
            token_network_deposit_limit,
        ).call()
    with pytest.raises(TransactionFailed, match="invalid token contract"):
        token_network_registry_contract.functions.createERC20TokenNetwork(
    ...
    with pytest.raises(TransactionFailed, match="invalid participant limit"):
        token_network_registry_contract.functions.createERC20TokenNetwork(
            custom_token.address, 0, token_network_deposit_limit
        ).call({"from": DEPLOYER_ADDRESS})

    with pytest.raises(TransactionFailed, match="invalid network deposit limit"):
        token_network_registry_contract.functions.createERC20TokenNetwork(
            custom_token.address, channel_participant_deposit_limit, 0
        ).call({"from": DEPLOYER_ADDRESS})

    with pytest.raises(TransactionFailed, match="invalid deposit limits"):
